...
internal abstract class ElasticsearchServiceBase
{
    ...
    protected void LogFailedElasticResponse(ILogger logger, string indexAlias, string message, ElasticsearchResponse response)
        => logger.LogError("{message}. Elastic index: {indexAlias}. Debug info from Elastic: {debugInformation}", message, indexAlias, response.DebugInformation);
}